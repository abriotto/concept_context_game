...
def plot_heatmap(result_list,
    ...
    for i in range(np.prod(plot_dims)):
        ...
        plt.ylabel('# attributes', fontsize=fontsize-2)
        plt.xticks(ticks=[0, 1, 2], labels=[4, 8, 16], fontsize=fontsize-3)
        plt.yticks(ticks=[0, 1, 2], labels=[3, 4, 5], fontsize=fontsize-3)
